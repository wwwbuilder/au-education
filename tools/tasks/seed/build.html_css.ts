...
function processComponentStylesheets() {
  return ENABLE_SCSS ? processComponentLess() : processComponentCss();
}
...
function errorHandler (title: any) {
 ...
 * Process scss files referenced from Angular component `styleUrls` metadata
 */
function processComponentLess() {
  return gulp.src(join(APP_SRC, 'assets/scss', '*.less'))
    // .pipe(isProd ? plugins.cached('process-component-scss') : plugins.util.noop())
    // .pipe(isProd ? plugins.progeny() : plugins.util.noop())
    .pipe(plugins.sourcemaps.init())
    // .pipe(plugins.less({paths: ['./node_modules/']}).on('error', errorHandler('Less')))
    .pipe(plugins.less()).on('error', errorHandler('Less'))
    .pipe(plugins.postcss(processors))
    .pipe(plugins.sourcemaps.write(isProd ? '.' : ''))
    .pipe(gulp.dest(isProd ? TMP_DIR : join(APP_DEST, 'assets/scss')));
}
...
export = () => merge(processComponentStylesheets(), prepareTemplates());
